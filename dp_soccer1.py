    ...
    def detect_goal(self, hsv_frame, detection_image):
        ...
        if contours:
            ...
            if area > 200:  # Minimum area threshold
                ...
                if not prev_detected:
                    self.get_logger().info(f"Goal detected! Area: {area:.0f} at ({x+w//2}, {y+h//2})")
...
def main(args=None):
    ...
    print("\n" + "="*60)
    print("⚽ SIMPLIFIED ROBOT SOCCER PLAYER ⚽")
    print("="*60)
    print("📋 FEATURES:")
    print("  • Red ball detection and tracking")
    print("  • Blue goal detection")
    print("  • Orange opponent detection")
    print("  • Basic soccer strategies")
    print("="*60)
    print("🔧 CONTROLS:")
